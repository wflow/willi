...
RELEASE_NAME := lilli-$(VERSION)-$(GOARCH)$(GOARM)
...
build-release:
	go build -o ../lilli -v -ldflags="-s -w -X main.version=$(VERSION)"

release: build-release
	rm -rf release/$(RELEASE_NAME)
	mkdir -p release/$(RELEASE_NAME)
	cp -r install.sh lilli lilli.service *.example release/$(RELEASE_NAME)/
